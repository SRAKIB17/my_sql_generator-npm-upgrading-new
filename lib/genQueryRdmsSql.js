 ...
 */
const genQueryRdmsSql = ({ table_list = {
    table1: "",
    table2: "",
    table3: '',
    table4: '',
    ...
    "table6": [],
    "table7": [],
}, limitSkip = { limit: '', skip: '' }, condition = '', sort, havingCondition = '', groupBY = [], min = '', max = '', count = '', sum = "" }) => {
    ...
    if (Object.values(specific_column).flat().length) {
        get_specif_field = Object.entries(specific_column).map((sf) => {
            const table = sf?.[0];
            const table_all_list = table_list;
